...
class SearchTableDiceRoll extends DiceRoll {
    ...
    async getTableData(tableName) {
        ...
        if (!checkDataTable)
            checkDataTable = await DB.db.getUserTable(
                this.channelID,
                tableName
            );
        if (!checkDataTable)
            checkDataTable = await DB.db.getUserTable(null, tableName);
    ...
    async checkTable(string) {
        ...
        //ただし、wasshoiまたはns?rsの文字が最初にある場合は検索する。（wasshoiエントリー、NRS発狂、NSRS発狂）
        //検索スキップにより、パフォーマンスを向上させる。
        if (
            !/^[a-zA-Z\d]/.test(tableName) ||
            /^wasshoi/i.test(tableName) ||
            /^ns?rs/i.test(tableName)
        ) {
        ...
        if (checkDataTable) {
            ...
            if (/\+x/.test(diceStr)) {
                ...
                if (additionValue && !Number.isNaN(Number(additionValue))) {
                    if (Number(additionValue) >= 0)
                        diceStr = diceStr.replace(
                            "+x",
                            "+" + Math.floor(Number(additionValue))
                        );
                    else
                        diceStr = diceStr.replace(
                            "+x",
