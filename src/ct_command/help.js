...
**ただし、ダイスショートカットのいずれかに「__ ndm+x __」形式のダイスが登録されていた場合は、
（値全てが対応しているか判定できないため）登録データのチェックをスキップします。**
...
引数：
    ...
    (結果文字列) ...ダイスロールの出目に対応し、表示する結果。英数字、日本語可。
                    " "（半角スペース）と","（カンマ）を入力する場合は直前に"\\"でエスケープすること。
    [t or f] ...省略可。公開済みテーブルを参照する場合はt、チャンネル内のテーブルを参照する場合はf。デフォルトはf
\`\`\`
...
改行などの内容もほぼそのまま反映されます。（末尾のたくさんの改行はDiscordが自動的に取り除いてしまいます）
コマンド：publishTable で公開されたテーブルについて更新する場合は、末尾に "t" のフラグを付けてください。
`,
...
引数：
    ...
    (ダイス文字列) ...表を参照した時のダイスロール。 ndm(+/- x or ndm)[+x]で表記する。例：1d6 , 3d6-3 , 4d6+3d10-5d6+32 , 3d6+x
    [t or f] ...省略可。公開済みテーブルを参照する場合はt、チャンネル内のテーブルを参照する場合はf。デフォルトはf
\`\`\`
...
ダイス文字列の末尾に「+x」をつけると、そのテーブルは表参照時に任意の整数値をつけてダイスロールできるようになります。
任意の整数値をつけたダイスロールの結果がデータの範囲外となった場合、結果に応じてデータ範囲の最大値or最小値を表示します。
