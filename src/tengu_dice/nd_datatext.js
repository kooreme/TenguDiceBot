...
（「/ndf ジツ系統決定#n 」（nは1～5）を入力すると入手できるジツの詳細を確認できます）
`,
]
...
シナリオ開始時ごとにD6を振り、偶数なら【精神力】が+1、奇数なら【精神力】が-1される（これにより【精神力】が0未満になる場合は、代わりに【体力】を-1する）。
この修正はそのシナリオの間のみ有効。
...
出目が６だった場合、チームの評価が１段階向上するが、出目が１だった場合は慢心による誤字を指摘され評価が１段階下がる（どちらの場合も評価リストの上限と下限を超えることはない）。
...
このスキルを持つ者は、取得条件を満たしている好きなワザ系スキル（◉）1個、もしくは自らのジツ系統に対応したジツ強化系スキル（☆◉）1個を持った状態でゲームを開始できる。
ジツ強化系スキルの取得条件を満たすために、他の能力値いずれかを合計-２することで、【ジツ】値を+1してもよい（最大で【ジツ】値3まで）。
...
※『風流判定』は【ワザマエ】のみを用いる特殊な判定であり、サイバネによるボーナスを一切得られない。
